...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://sandy:sAd#26102002@localhost/inventory_db'
...
class ProductMovement(db.Model):
    ...
    fromLoc = db.relationship('Location', foreign_keys=[from_location], uselist=False, primaryjoin="ProductMovement.from_location == Location.location_id")
    toLoc = db.relationship('Location', foreign_keys=[to_location], uselist=False, primaryjoin="ProductMovement.to_location == Location.location_id")
...
def viewMovements():
    if request.method == "POST":
        ...
        new_movement = ProductMovement(product_id=product_id, qty=qty, from_location=fromLocation, to_location=toLocation)
        try:
            db.session.add(new_movement)
            db.session.commit()
            return redirect("/movements/")
        except:
            return "There Was an issue while adding a new Movement"
    else:
        products = Product.query.order_by(Product.date_created).all()
        locations = Location.query.order_by(Location.date_created).all()
...
def getLocations():
    ...
    for key, loc in enumerate(locations):
        if locationDict[loc.to_location] and locationDict[loc.to_location]["qty"]:
            locationDict[loc.to_location]["qty"] += loc.qty
        else:
            locationDict[loc.to_location]["qty"] = loc.qty
    return locationDict
...
def getDublicate():
    location = request.form["location"]
    locations = Location.query.filter(Location.location_id == location).all()
    if locations:
        return {"output": False}
    else:
        return {"output": True}
...
def getPDublicate():
    product_name = request.form["product_name"]
    products = Product.query.filter(Product.product_id == product_name).all()
    if products:
        return {"output": False}
    else:
        return {"output": True}
